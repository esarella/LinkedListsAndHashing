...
public class TwoSum {
    public static ArrayList<Integer> twoSum(final List<Integer> a, int b) {
        ...
        for(int i = 0; i < a.size(); i++){
            if(hashMap.containsKey(b-a.get(i))){
                result = hashMap.get(b-a.get(i));
                result.add(i+1);
                hashMap.put(b-a.get(i), result);
                return result;
    ...
    public static void main(String[] args) {
        ...
        ArrayList<Integer> result = twoSum(list, 9);

        if (result.size() == 2){
            System.out.println("Index1 = "+ result.get(0)+ " , " + "Index2 =  " + result.get(1));
        }
    }
}
